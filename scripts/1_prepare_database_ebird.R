...
first_lines <- readLines(here("data", "raw", "ebd_ES_201612_202601_smp_relJan-2026", "ebd_ES_201612_202601_smp_relJan-2026.txt"), 
                         n = 5)
print(first_lines)
...
ebd <- auk_ebd(
  here("data", "raw", "ebd_ES_201612_202601_smp_relJan-2026", "ebd_ES_201612_202601_smp_relJan-2026.txt"),
  file_sampling = here("data", "raw", "ebd_sampling_relJan-2026", "ebd_sampling_relJan-2026.txt")
)
...
for (sp_name in names(species_list)) {
  ...
  ebd_zf <- ebd_zf %>%
    mutate(
      ...
      effort_distance_km       = if_else(protocol_name != "Traveling", 0, effort_distance_km),
  ...
  ebd_zf_filtered <- ebd_zf %>%
    filter(
      ...
      year >= 2017,
      year <= 2023,          # landcover data available up to 2023
      number_observers   <= 10
    )
