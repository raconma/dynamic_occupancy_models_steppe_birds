...
#                    gammaformula = ~ Land_Cover_Type_1_Percent_Class_7 + Land_Cover_Type_1_Percent_Class_13 + NDVI + tmmx, # pr
...
# 1597.125
Mod.final <- colext(psiformula = ~ bio2 + tree_cover + grass_cover, 
                    gammaformula = ~ Land_Cover_Type_1_Percent_Class_7 + NDVI + tmmn + tmmx,
                    epsilonformula = ~ Land_Cover_Type_1_Percent_Class_7 + Land_Cover_Type_1_Percent_Class_12 +
                      Land_Cover_Type_1_Percent_Class_13 + NDVI + pr,
                    pformula = ~ time + duration + observers + pr_obs + topo_aspect_obs,
                    data = occ_umf)
...
for(i in 1:T){
  ...
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_7,NDVI,tmmn,tmmx) %>% 
    drop_na() %>% 
    scale(.) %>% 
    as.data.frame(.)
...
for(i in 1:T){
  ...
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_7, Land_Cover_Type_1_Percent_Class_12,
                 Land_Cover_Type_1_Percent_Class_13,NDVI,pr) %>% 
    drop_na() %>% 
    scale(.) %>% 
    as.data.frame(.)
