...
f_ebd <- file.path(data_dir, "ebd_falnau_breeding.txt")
f_sampling <- file.path(data_dir, "ebd_checklists_breeding_falnau.txt")
...
write.csv(ebird[1:(nrow(ebird)/2), ], "../../data/falnau/ebd_falnau_breeding_spain_zf_part1.csv", na = "", row.names = FALSE)
write.csv(ebird[(nrow(ebird)/2 + 1):nrow(ebird), ], "../../data/falnau/ebd_falnau_breeding_spain_zf_part2.csv", na = "", row.names = FALSE)
