...
occ_wide_clean <- read.csv("../data/cirpyg/cirpyg_occ_wide_dynamic.csv")

# Plot variable names
names(occ_wide_clean)

# Define the survey covariates
...
Mod.final <- colext(psiformula = ~ bio12 + tree_cover + grass_cover, 
                    gammaformula = ~ Land_Cover_Type_1_Percent_Class_0 + Land_Cover_Type_1_Percent_Class_13 + NDVI + pr + tmmn + tmmx, 
                    epsilonformula = ~ Land_Cover_Type_1_Percent_Class_0 + Land_Cover_Type_1_Percent_Class_10 + Land_Cover_Type_1_Percent_Class_12 + Land_Cover_Type_1_Percent_Class_13 + NDVI + pr, 
...
variables_selection <- c("bio12", "tree_cover", "grass_cover")
...
pred_surface_std <- pred_surface %>%
  mutate_at(c("bio12", "tree_cover", "grass_cover"), ~(scale(.) %>% as.vector))

occ_pred <- predict(model_occ, 
                    newdata = as.data.frame(pred_surface_std[,c("bio12", "tree_cover", "grass_cover")]), type = "psi")
...
#save result
write.csv(pred_occ,here("../data/cirpyg/occ_cirpyg_prediction.csv"), row.names = FALSE)

pred_occ <- read_csv(here("../data/cirpyg/occ_cirpyg_prediction.csv"))
...
writeRaster(raster_initial, "../data/cirpyg/cirpyg_OccuMap.tif", format = "GTiff", overwrite = TRUE)
...
psi1<-predict(model_prediction, type="psi",newdata=as.data.frame(predict_data[,c("bio12", 
                                                                                 "tree_cover",
...
for(i in 1:T){
  ...
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_0,
                 Land_Cover_Type_1_Percent_Class_13,NDVI,pr,tmmn,tmmx) %>% 
    drop_na() %>% 
    scale(.) %>% 
    as.data.frame(.)
...
for(i in 1:T){
  ...
  NDVI <- as.data.frame(occ_wide_clean[,c(paste0('NDVI_',(pp[i])))])
  pr <- as.data.frame(occ_wide_clean[,c(paste0('pr_',(pp[i])))])  
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_0,Land_Cover_Type_1_Percent_Class_10,
