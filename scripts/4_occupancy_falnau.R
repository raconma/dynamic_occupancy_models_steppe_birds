...
occ_wide_clean <- read.csv("../data/falnau/falnau_occ_wide_dynamic.csv")
...
Mod.final <- colext(psiformula = ~ bio2 + bio12 + tree_cover + grass_cover, 
                    gammaformula = ~ Land_Cover_Type_1_Percent_Class_0 + Land_Cover_Type_1_Percent_Class_6 + Land_Cover_Type_1_Percent_Class_7 + Land_Cover_Type_1_Percent_Class_12 + pr + tmmx,
                    epsilonformula = ~ Land_Cover_Type_1_Percent_Class_0 + Land_Cover_Type_1_Percent_Class_10 + Land_Cover_Type_1_Percent_Class_13 + tmmn, 
                    pformula = ~ duration + effort, 
                    data = occ_umf)
...
variables_selection <- c("bio2","bio12", "tree_cover", "grass_cover")
...
pred_surface <- pred_surface %>% 
  drop_na(bio2, tree_cover, bio12, grass_cover)
...
pred_surface_std <- pred_surface %>%
  mutate_at(c("bio2", "bio12", "tree_cover", "grass_cover"), ~(scale(.) %>% as.vector))

occ_pred <- predict(model_occ, 
                    newdata = as.data.frame(pred_surface_std[,c("bio2", "bio12", "tree_cover", "grass_cover")]), type = "psi")
...
#save result
write.csv(pred_occ,here("../data/falnau/occ_falnau_prediction.csv"), row.names = FALSE)

pred_occ <- read_csv(here("../data/falnau/occ_falnau_prediction.csv"))
...
writeRaster(raster_initial, "../data/falnau/falnau_OccuMap.tif", format = "GTiff", overwrite = TRUE)
...
for(i in 1:T){
  ...
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_0,Land_Cover_Type_1_Percent_Class_6,
                 Land_Cover_Type_1_Percent_Class_7,Land_Cover_Type_1_Percent_Class_12,pr,tmmx) %>% 
    drop_na() %>% 
    scale(.) %>% 
    as.data.frame(.)
...
for(i in 1:T){
  ...
  #spXdets_sdf<-detGGA_sdf[detGGA_sdf$CommonName ==speciesName,]
  Land_Cover_Type_1_Percent_Class_0 <- as.data.frame(occ_wide_clean[,c(paste0('Land_Cover_Type_1_Percent_Class_0_',(pp[i])))])
  Land_Cover_Type_1_Percent_Class_10 <- as.data.frame(occ_wide_clean[,c(paste0('Land_Cover_Type_1_Percent_Class_10_',(pp[i])))])  
  ...
  new.dat<-cbind(Land_Cover_Type_1_Percent_Class_0,Land_Cover_Type_1_Percent_Class_10,Land_Cover_Type_1_Percent_Class_13,tmmn) %>% 
    drop_na() %>% 
    scale(.) %>% 
    as.data.frame(.)
