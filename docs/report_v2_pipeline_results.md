...
## 8. Que significa esto para el TFM

### 8.1 Mensaje principal

**Los modelos originales no eran incorrectos conceptualmente, pero los datos no tenian la estructura temporal necesaria para estimarlos.** El pipeline v2 demuestra que con un filtrado adecuado, los mismos datos eBird producen resultados biologicamente plausibles.

### 8.2 Que se puede publicar

Con los resultados del pipeline v2 se puede:
...
3. **Discutir las implicaciones para estudios con datos eBird**: muchos trabajos usan `filter_repeat_visits` sin considerar las implicaciones para modelos multi-temporales.

### 8.3 Limitaciones actuales

1. **Sin variables dinamicas**: El pipeline v2 solo usa covariables estaticas porque el cambio de sitios invalida el export original de Google Earth Engine. Para la version final, habria que re-exportar las variables dinamicas (NDVI, temperatura, precipitacion, land cover) para los nuevos sitios.

2. **Colonizacion/extincion solo como intercepto**: Los modelos v2 no incluyen covariables en gamma y epsilon. Esto es una decision deliberada: primero demostrar que los interceptos son plausibles antes de anadir complejidad.

3. **Solo ejecutado para *Otis tarda***: Los datos crudos de las otras 3 especies necesitan descargarse de iCloud para ejecutar el pipeline v2.

---

## 9. Proximos pasos

### 9.1 Inmediatos (1-2 semanas)

1. **Descargar los datos de iCloud** para ptealc, pteori y tettet
2. **Ejecutar el pipeline v2** para las 4 especies (script: `scripts/test_pipeline_v2_all_species.R`)
3. **Verificar resultados** para las 4 especies

### 9.2 Corto plazo (2-4 semanas)

4. **Re-exportar variables dinamicas desde GEE** para los nuevos sitios filtrados
5. **Incorporar covariables en gamma y epsilon** (NDVI, land cover, temperatura)
6. **Seleccion de modelos** con AIC/BIC para cada especie

### 9.3 Medio plazo (1-2 meses)

7. **Goodness-of-fit (GOF)**: Validar los modelos con bootstrap parametrico
8. **Curvas de respuesta y mapas de prediccion**
9. **Simulaciones estocasticas** de trayectorias de ocupacion
10. **Redactar manuscrito/TFM**
...
| `scripts/test_pipeline_v2_all_species.R` | Pipeline v2 multi-especie (pendiente de datos iCloud) |
...
# 2. Las 4 especies (necesita que los datos esten descargados de iCloud):
